...
_0x0:
	...
	.DB  0x4D,0x49,0x3A,0x0,0x45,0x6E,0x74,0x65
	.DB  0x72,0x20,0x54,0x65,0x6D,0x70,0x3A,0x20
	.DB  0x0,0x45,0x6E,0x74,0x65,0x72,0x20,0x48
	.DB  0x75,0x6D,0x69,0x64,0x69,0x74,0x79,0x3A
	.DB  0x20,0x0,0x53,0x41,0x59,0x20,0x58,0x4F
	.DB  0x4E,0x47,0x21,0x0
_0x2060003:
	.DB  0x80,0xC0
_0x2080060:
	.DB  0x1
...
; 0000 0087     x = dht11(&nhietdo, &doam);
	...
	MOV  R5,R30
; 0000 0088 
; 0000 0089     // Hi?n th? nhi?t ð?
; 0000 008A     glcd_clear();  // Xóa màn h?nh GLCD trý?c khi hi?n th?
	CALL SUBOPT_0x0
; 0000 008B     glcd_outtextxy(0, 0, "TEMP:");  // Hi?n th? nh?n "TEMP"
	__POINTW2MN _0x43,0
	CALL _glcd_outtextxy
; 0000 008C     glcd_putchar(48 + nhietdo / 10);  // Hi?n th? ch? s? ð?u tiên c?a nhi?t ð?
	MOV  R26,R4
	CALL SUBOPT_0x1
; 0000 008D     glcd_putchar(48 + nhietdo % 10);  // Hi?n th? ch? s? th? hai c?a nhi?t ð?
	MOV  R26,R4
	CALL SUBOPT_0x2
; 0000 008E 
; 0000 008F     // Hi?n th? ð? ?m
; 0000 0090     glcd_outtextxy(0, 10, "HUMI:");  // Hi?n th? nh?n "HUMI"
	...
	__POINTW2MN _0x43,6
	CALL _glcd_outtextxy
; 0000 0091     glcd_putchar(48 + doam / 10);  // Hi?n th? ch? s? ð?u tiên c?a ð? ?m
	MOV  R26,R7
	CALL SUBOPT_0x1
; 0000 0092     glcd_putchar(48 + doam % 10);  // Hi?n th? ch? s? th? hai c?a ð? ?m
	MOV  R26,R7
	CALL SUBOPT_0x2
; 0000 0093 }
	RET
...
_0x45:
	...
	BRLT PC+2
	RJMP _0x46
; 0000 009A         // Kích ho?t t?ng hàng c?a bàn phím
...
; 0000 00A3                 if (a != 0x04) {
	CPI  R17,4
	BREQ _0x4E
; 0000 00A4                     input_buffer[buffer_index++] = keypad[j][i] + '0'; // Chuy?n s? sang k? t? ð? lýu vào b? ð?m
	CALL SUBOPT_0x3
; 0000 00A5                 }
; 0000 00A6             }
_0x4E:
...
; 0000 00BB     init_glcd(4, 66);  // Kh?i t?o GLCD
	...
	LDI  R27,0
	RCALL _init_glcd
; 0000 00BC     init_register();   // Kh?i t?o các thanh ghi và ph?n c?ng
...
; 0000 00BF     while (1) {
_0x53:
; 0000 00C0         BUTTON();  // Ki?m tra bàn phím ma tr?n
	RCALL _BUTTON
; 0000 00C1         delay_ms(250);
	CALL SUBOPT_0x4
; 0000 00C2 
; 0000 00C3         if (!(PINB & (1 << PINB2))) {  // Button 1 ðý?c nh?n
	SBIC 0x16,2
	RJMP _0x56
; 0000 00C4             // Nh?p giá tr? nhi?t ð? và ð? ?m t? bàn phím
; 0000 00C5             if (PINB.2 == 0) // N?u button 1 ðý?c nh?n
; 0000 00C6             {
; 0000 00C7                 // break
; 0000 00C8             }
...
; 0000 00CA             glcd_outtextxy(0, 0, "Enter Temp: ");
	__POINTW2MN _0x58,0
	CALL _glcd_outtextxy
; 0000 00CB             // Ð?c giá tr? t? bàn phím và chuy?n vào `input_buffer`
; 0000 00CC             while (buffer_index < 4) {  // Ch? cho phép nh?p t?i ða 2 ch? s?
_0x59:
	LDS  R26,_buffer_index
	LDS  R27,_buffer_index+1
	SBIW R26,4
	BRLT _0x59
; 0000 00CD                 // N?u ð? 4 ch? s?, nh?p vào buffer
; 0000 00CE             }
; 0000 00CF             nhietdo = atoi(input_buffer);  // Chuy?n chu?i thành s?
...
; 0000 00D0             glcd_outtextxy(0, 10, "Enter Humidity: ");
	...
	LDI  R30,LOW(10)
	ST   -Y,R30
	__POINTW2MN _0x58,13
	CALL _glcd_outtextxy
; 0000 00D1             buffer_index = 0;  // Reset b? ð?m
	LDI  R30,LOW(0)
...
; 0000 00D3                 // Nh?p ð? ?m
; 0000 00D4             }
; 0000 00D5             doam = atoi(input_buffer);  // Chuy?n chu?i thành s?
	LDI  R26,LOW(_input_buffer)
	LDI  R27,HIGH(_input_buffer)
	CALL _atoi
	MOV  R7,R30
; 0000 00D6         }
; 0000 00D7 
; 0000 00D8         if (!(PINB & (1 << PINB3))) {  // Button 2 ðý?c nh?n
_0x56:
	SBIC 0x16,3
	RJMP _0x5F
; 0000 00D9             hien_thi_glcd();
	RCALL _hien_thi_glcd
; 0000 00DA             // Hi?n th? nhi?t ð? và ð? ?m t? DHT11
